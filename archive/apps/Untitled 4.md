...
// Initialize all muscles as 100% Fresh (Green)
for (const k in muscleRegistry) {
    muscleRegistry[k].recovery = 100;
}

// ============================================================================
// 2. BIOMECHANICS & MICRO-MUSCLE DATABASE (MAPPED TO REGISTRY KEYS)
// ============================================================================
const exerciseDB = [
  // CHEST
...
// ============================================================================
// 3. COLOR TIERS & THEME CONFIGURATION
// ============================================================================
const HEAT_TIERS = {
...
styleSheet.textContent = `
  ...
  /* ANATOMICAL VECTOR HEATMAP SECTION */
  .bm3-root {
...
// ============================================================================
// 6. DYNAMIC FATIGUE ENGINE (CONNECTS LOGGED EXERCISES TO HEATMAP)
// ============================================================================
function recalculateMuscleFatigue() {
  if (activePresetKeys) return;

  // Reset to fresh green
  for (const k in muscleRegistry) {
    muscleRegistry[k].recovery = 100;
  }

  // Count done sets targeting each muscle key
  const setCounts = {};
  ...
  // Calculate sore levels
  for (const k in setCounts) {
    if (muscleRegistry[k]) {
      const c = setCounts[k];
      const baseHours = muscleRegistry[k].defaultHours || 48;
      if (c === 1) {
        muscleRegistry[k].recovery = 65; // Yellow
      } else if (c === 2) {
        muscleRegistry[k].recovery = 40; // Orange
      } else {
        muscleRegistry[k].recovery = baseHours >= 72 ? 15 : 25; // Red
      }
    }
  }
...
function initActiveApp() {
  ...
  trackerApp.querySelector("#btn-save-note").onclick = async () => {
    ...
    let markdownLog = `\n\n### 🏋️ Saved Workout Log - ${new Date().toISOString().slice(0, 10)}\n`;
    sessionExercises.forEach(ex => {
    ...
    await app.vault.append(activeFile, markdownLog);
    new Notice("✅ Workout saved to vault note!");
  };
}
