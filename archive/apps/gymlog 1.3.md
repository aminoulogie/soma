...
// ============================================================================
// 3. MASTER UNIFIED STYLESHEET (MATCHING EXACT WIDTH FOR BOTH CARDS)
// ============================================================================
const masterRoot = dv.el("div", "", { cls: "unified-gym-workspace" });
const styleEl = document.createElement("style");
styleEl.textContent = `
    .unified-gym-workspace {
    ...
    /* ========================================================================
       BOTTOM SEPARATE HEATMAP COMPONENT (IMAGE 2 - MATCHED WIDTH)
       ======================================================================== */
    .bm3-root {
    ...
    .bm3-header { text-align: center; margin-bottom: 16px; position: relative; }
    .bm3-title { font-size: 1.45rem; font-weight: 800; color: #f1f5f9; display: flex; align-items: center; justify-content: center; gap: 10px; }
    .bm3-title .bm3-icon { color: #38bdf8; }
    .bm3-subtitle { color: #64748b; font-size: 0.82rem; margin-top: 6px; }

    .bm3-viewtabs { display: flex; justify-content: center; gap: 6px; margin: 16px 0 8px; }
    .bm3-viewtab {
    ...
    .bm3-layout { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 8px 0; }
    ...
    .bm3-legend-row { display: flex; align-items: center; gap: 8px; margin-bottom: 11px; }
    ...
    .bm3-howto-row { display: flex; align-items: flex-start; gap: 8px; margin-bottom: 14px; }
    ...
    .bm3-detail-hours { color: #38bdf8; font-size: 0.82rem; font-weight: 700; margin: 4px 0 6px; display: flex; align-items: center; gap: 6px; }
    .bm3-detail-desc { color: #94a3b8; font-size: 0.82rem; max-width: 440px; line-height: 1.4; }
    .bm3-detail-tag {
        padding: 9px 16px; border-radius: 10px; font-weight: 700; font-size: 0.8rem;
        color: #fff; white-space: nowrap; flex-shrink: 0;
    }

    .bm3-presets { margin-top: 20px; }
    .bm3-presets-label { font-size: 0.68rem; letter-spacing: 0.08em; color: #64748b; font-weight: 700; margin-bottom: 10px; }
    .bm3-preset-row { display: flex; gap: 8px; flex-wrap: wrap; }
    .bm3-preset-btn {
        background: rgba(148,163,184,0.08); border: 1px solid rgba(148,163,184,0.15);
        color: #cbd5e1; padding: 8px 14px; border-radius: 10px; font-size: 0.78rem; font-weight: 600;
...
`;
masterRoot.appendChild(styleEl);
...
function renderBottomHeatmap(containerEl, postWorkoutSoreness = null) {
    const INSTANCE_ID = Math.random().toString(36).slice(2, 9);
    const muscleRegistry = JSON.parse(JSON.stringify(rawMuscleRegistry));

    // Configure initial recovery state:
    // BEFORE workout: All muscles 100% at rest (Fresh)
    // AFTER workout: sore muscles receive their calculated recovery scores and windows
    Object.keys(muscleRegistry).forEach(k => {
        if (postWorkoutSoreness && postWorkoutSoreness[k]) {
            ...
            muscleRegistry[k].recovery = 95; // All at rest
    ...
    const rootEl = containerEl.createDiv({ cls: "bm3-root" });
...
// ============================================================================
// 5. WORKOUT TRACKER STATE & CORE LOGIC
// ============================================================================
let savedRecap = null;
try {
...
// ============================================================================
// 6. POST-WORKOUT FINISHED SCREEN (DYNAMIC SORENESS ON HEATMAP)
// ============================================================================
function renderFinishedScreen(data) {
  if (durationInterval) clearInterval(durationInterval);
  masterRoot.empty();

  const topTrackerCard = masterRoot.createDiv({ cls: "wk-app" });
  let cardsHtml = "";
  const soreMuscles = {};
  ...
  data.exercises.forEach(ex => {
    ...
    // Register soreness for bottom heatmap
    if (info.targetKeys) {
      info.targetKeys.forEach(k => {
        ...
        soreMuscles[k] = {
          ...
          note: `Fatigued from ${ex.name} (${info.subTarget || ex.muscle}). Direct hypertrophy strain.`
        };
      });
    }
  ...
  // Render bottom heatmap with post-workout sore muscle highlights
  renderBottomHeatmap(masterRoot, soreMuscles);
}
...
function initActiveApp() {
  ...
  renderBottomHeatmap(masterRoot, null);
}
