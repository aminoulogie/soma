...
const baseMuscleRegistry = JSON.parse(registryContent);
...
// ============================================================================
// 2. EXERCISE DATABASE & ANATOMICAL REGISTRY MAPPING
// ============================================================================
const exerciseDB = [
  // CHEST
...
// ============================================================================
// 3. MASTER STYLESHEET (TOP TRACKER + BOTTOM SEPARATE HEATMAP)
// ============================================================================
...
styleEl.textContent = `
    ...
    /* TOP GYM LOG CARD (IMAGE 1 EXACT DESIGN) */
    .wk-app {
        background: #070d19;
    ...
    /* ========================================================================
       BOTTOM SEPARATE HEATMAP COMPONENT (IMAGE 2 EXACT DESIGN)
       ======================================================================== */
    .bm3-root {
    ...
    .bm3-layout { display: flex; align-items: center; justify-content: center; gap: 16px; flex-wrap: wrap; padding: 8px 0; }
    ...
    .bm3-detail-desc { color: #94a3b8; font-size: 0.85rem; max-width: 520px; line-height: 1.4; }
    .bm3-detail-tag {
        padding: 10px 18px; border-radius: 10px; font-weight: 700; font-size: 0.82rem;
        color: #fff; white-space: nowrap; flex-shrink: 0;
    }

    .bm3-presets { margin-top: 22px; }
    ...
    .bm3-preset-btn {
        ...
        color: #cbd5e1; padding: 9px 15px; border-radius: 10px; font-size: 0.8rem; font-weight: 600;
...
// ============================================================================
// 4. HEATMAP ENGINE & COMPONENT BUILDER
// ============================================================================
const HEAT_TIERS = {
...
function renderBottomHeatmap(containerEl, postWorkoutSoreness = null) {
    ...
    const muscleRegistry = JSON.parse(JSON.stringify(baseMuscleRegistry));
    ...
    Object.keys(muscleRegistry).forEach(k => {
        if (postWorkoutSoreness && postWorkoutSoreness[k]) {
            ...
            muscleRegistry[k].sessionNote = "Muscle at baseline rest. Fully ready for loading.";
        }
    });
    ...
    header.createDiv({
        ...
        text: postWorkoutSoreness 
            ? "Post-Workout Recovery Status • Click any muscle to lock remaining recovery time" 
...
// ============================================================================
// 5. TOP TRACKER STATE & APP RUNTIME
// ============================================================================
let savedRecap = null;
try {
...
// ============================================================================
// 6. POST-WORKOUT FINISHED RECAP SCREEN (DYNAMIC SORENESS ON HEATMAP)
// ============================================================================
function renderFinishedScreen(data) {
  ...
  // Render bottom heatmap with post-workout fatigue
  renderBottomHeatmap(masterRoot, soreMuscles);
}

// ============================================================================
// 7. ACTIVE WORKOUT APP (BODY MAP AT BOTTOM IS ALL AT REST)
// ============================================================================
function initActiveApp() {
  ...
  // MOUNT SEPARATE HEATMAP AT BOTTOM (ALL AT REST BEFORE WORKOUT)
