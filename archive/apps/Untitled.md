...
// ============================================================================
// 3. UNIFIED CONTAINER & EXACT 640PX WIDTH CSS
// ============================================================================
...
styleEl.textContent = `
    ...
    /* Barbell Plate Modal */
    .wk-plate-modal { display: none; position: fixed; z-index: 9999; background: #0b1324; border: 1px solid #3b82f6; border-radius: 14px; padding: 14px; box-shadow: 0 12px 35px rgba(0,0,0,0.85); width: 260px; left: 50%; top: 50%; transform: translate(-50%, -50%); }
...
function initActiveApp() {
  ...
  topTrackerCard.querySelector("#btn-save-note").onclick = async () => {
    ...
    sessionExercises.forEach(ex => {
      ...
      const safeSub = String(ex.subTarget || ex.muscle).replace(/[\[\]\(\)]/g, "");
      ...
      ex.sets.forEach(s => {
        ...
        markdownLog += `- ${safeName} | ${w}kg × ${r} reps (Level ${failVal}) ${s.done ? "✅" : "⏳"}\n`;
      });
    ...
    markdownLog += `\n- **Summary:** Duration: ${durationFormatted} | Burn: ${caloriesBurned} kcal | Volume: ${totalVol} kg | Sets: ${totalSets}\n`;
    ...
    await app.vault.modify(activeFile, updatedContent + markdownLog);
...
**Incline Dumbbell Press** (Upper Pec)
- Incline Dumbbell Press | 80kg × 8 reps (Level 3) ✅
- Incline Dumbbell Press | 80kg × 8 reps (Level 3) ✅
- Incline Dumbbell Press | 80kg × 8 reps (Level 3) ✅

- **Summary:** Duration: 0m 8s | Burn: 16 kcal | Volume: 1920 kg | Sets: 3
