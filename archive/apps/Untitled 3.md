...
const routinePresets = {
  "Push (Chest/Delts/Triceps)": [
    ...
    "Cable Lateral Raise",
    "Overhead Dual Cable Extension",
    "Cable Triceps Pushdown (Straight/V)"
  ],
...
styleEl.textContent = `
    ...
    .wk-btn.active { background: #2563eb; border-color: #60a5fa; color: #fff; }
...
function wrapPath(content, className = "", extraAttrs = "") {
    if (!content) return "";
    const trimmed = String(content).trim();
    if (trimmed.startsWith("<")) return trimmed;
    return `<path class="${className}" d="${trimmed}" ${extraAttrs} />`;
}
...
function renderSomaVisualizer() {
    ...
    // Generate dynamic muscle elements from muscleRegistry
    function buildMusclePaths() {
        let pathsHtml = "";
        if (Array.isArray(rawMuscleRegistry)) {
            rawMuscleRegistry.forEach(m => {
                const key = m.key || m.id || m.targetKey || m.name;
                const d = m.d || m.path || (typeof m === "string" ? m : "");
                if (d) {
                    const isActive = activeKeys.has(key);
                    pathsHtml += `<path data-muscle="${key}" d="${d}" class="soma-muscle ${isActive ? 'soma-active-muscle' : ''}" />`;
                }
            });
        } else if (typeof rawMuscleRegistry === "object" && rawMuscleRegistry !== null) {
            for (const [key, val] of Object.entries(rawMuscleRegistry)) {
                const d = typeof val === "string" ? val : (val.d || val.path || "");
                if (d) {
                    const isActive = activeKeys.has(key);
                    pathsHtml += `<path data-muscle="${key}" d="${d}" class="soma-muscle ${isActive ? 'soma-active-muscle' : ''}" />`;
                }
    ...
    const musclePathsHtml = buildMusclePaths();

    function createSvg(viewBox, outlineD) {
        ...
        svg.innerHTML = `
            ${wrapPath(outlineD, "soma-outline")}
            ${wrapPath(STATIC_PARTS, "soma-static")}
            ${musclePathsHtml}
        `;
        return svg;
    }

    somaWrapper.appendChild(createSvg(FRONT_VIEWBOX, FRONT_OUTLINE));
    somaWrapper.appendChild(createSvg(BACK_VIEWBOX, BACK_OUTLINE));
}
    ...
    trackerContainer.innerHTML = `
        ...
        <div class="wk-timer-radial-dock">
            <div style="display:flex; align-items:center; gap:12px;">
                ...
                <div>
                    ...
                    <div style="font-size:0.65rem; color:#64748b;">Auto-recovery countdown</div>
                </div>
            </div>
        ...
        ${missingHeads.length > 0 ? `
            <div class="wk-gap-box">
                ...
                <div style="color: #cbd5e1; font-size: 0.72rem;">Target heads not yet covered in this ${currentSplit} session:</div>
        ...
        <div class="wk-action-row">
            <button class="wk-btn ${currentSplit === 'Push' ? 'active' : ''}" id="wk-btn-push">Push</button>
            <button class="wk-btn ${currentSplit === 'Pull' ? 'active' : ''}" id="wk-btn-pull">Pull</button>
            <button class="wk-btn ${currentSplit === 'Legs' ? 'active' : ''}" id="wk-btn-legs">Legs</button>
...
function loadPreset(presetKey, splitName) {
    ...
    const targetNames = routinePresets[presetKey] || [];
    activeExercises = exerciseDB
        .filter(ex => targetNames.includes(ex.name))
        .map(ex => ({
            ...ex,
            sets: [
...
// Initial session load (Push)
loadPreset("Push (Chest/Delts/Triceps)", "Push");
```