...
// ============================================================================
// 2. UI STYLES (ENHANCED FOR IOS TOUCH & FLUID PHYSICS)
// ============================================================================
...
style.textContent = `
  ...
  .ntr-card { background: #18181b; border: 1px solid #27272a; border-radius: 12px; overflow: hidden; }

  /* iOS FIX: Minimum 16px font-size to prevent automatic Safari zoom */
  ...
  /* TAP PHYSICS & BUTTONS */
  .ntr-btn, .ntr-btn-tool, .ntr-btn-water, .ntr-chip, .ntr-del-btn, .ntr-drop-glass {
    touch-action: manipulation;
    cursor: pointer;
    user-select: none;
    transition: transform 0.1s ease, background 0.15s ease, opacity 0.15s ease;
  }
  .ntr-btn:active, .ntr-btn-tool:active, .ntr-btn-water:active, .ntr-chip:active, .ntr-drop-glass:active {
    transform: scale(0.96);
  }

  /* TARGET LOCKED SHIMMER EFFECT */
  .goal-achieved-glow {
  ...
  /* HEADER & GOAL WIDGET */
  .ntr-goals-widget { background: linear-gradient(145deg, #18181b, #202024); border: 1px solid #2e2e34; border-radius: 12px; padding: 10px 14px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 8px; }
  .ntr-goals-info { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
  .ntr-goals-title { font-size: 0.74rem; font-weight: 800; color: #a1a1aa; text-transform: uppercase; letter-spacing: 0.06em; }
  .ntr-goal-pill { font-size: 0.76rem; font-weight: 800; padding: 3px 8px; border-radius: 6px; background: #27272a; display: inline-flex; align-items: center; gap: 4px; }
  .ntr-btn-edit-goals { background: #27272a; border: 1px solid #3f3f46; color: #cbd5e1; border-radius: 8px; padding: 6px 12px; font-size: 0.74rem; font-weight: 700; }
  .ntr-btn-edit-goals:hover { background: #3f3f46; color: #ffffff; }

  /* WORKOUT SYNERGY BADGE */
  .ntr-synergy-banner { background: rgba(59, 130, 246, 0.12); border: 1px solid rgba(59, 130, 246, 0.35); border-radius: 10px; padding: 8px 12px; font-size: 0.76rem; font-weight: 800; color: #60a5fa; display: flex; align-items: center; justify-content: space-between; gap: 8px; }

  /* MFP REMAINING CALORIE HERO BANNER */
  .ntr-mfp-banner { background: #131316; border: 1px solid #27272a; border-radius: 12px; padding: 12px 16px; display: flex; justify-content: space-between; align-items: center; }
  ...
  .ntr-mfp-num { font-size: 1.1rem; font-weight: 800; color: #ffffff; }
  ...
  .ntr-mfp-rem-box { background: #202023; border: 1px solid #2e2e34; padding: 6px 14px; border-radius: 8px; text-align: center; }
  .ntr-mfp-rem-val { font-size: 1.3rem; font-weight: 900; }

  /* SEGMENTED MACRO CALORIE BAR */
  .ntr-stacked-bar-container { background: #202023; border: 1px solid #2e2e32; border-radius: 10px; padding: 10px 12px; }
  .ntr-stacked-bar-track { height: 10px; border-radius: 999px; background: #27272a; display: flex; overflow: hidden; margin-top: 6px; }
  .ntr-stack-seg { height: 100%; transition: width 0.3s ease; }
  .ntr-stack-over { background: repeating-linear-gradient(45deg, #ef4444, #ef4444 6px, #991b1b 6px, #991b1b 12px); }

  /* INTERACTIVE WATER GLASS PROGRESSION */
  .ntr-water-dock { background: #0c1a2e; border: 1px solid #1e3a8a; border-radius: 12px; padding: 10px 14px; }
  .ntr-water-top { display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; }
  .ntr-water-label { font-size: 0.78rem; font-weight: 800; color: #38bdf8; display: flex; align-items: center; gap: 6px; }
  .ntr-water-glasses-row { display: flex; gap: 6px; justify-content: space-between; align-items: center; overflow-x: auto; padding: 2px 0; }
  .ntr-drop-glass { flex: 1; min-width: 26px; height: 32px; background: #172554; border: 1px solid #1e40af; border-radius: 6px; display: flex; align-items: center; justify-content: center; font-size: 0.75rem; }
  .ntr-drop-glass.filled { background: #0284c7; border-color: #38bdf8; box-shadow: 0 0 8px rgba(56, 189, 248, 0.4); }
  ...
  /* MEAL GROUPS */
  .ntr-meal-card { background: #1c1c1f; border-bottom: 1px solid #27272a; }
  .ntr-meal-header { padding: 9px 12px; background: #202024; display: flex; justify-content: space-between; align-items: center; cursor: pointer; user-select: none; border-top: 1px solid #27272a; }
  .ntr-meal-title { font-size: 0.8rem; font-weight: 800; color: #f4f4f5; display: flex; align-items: center; gap: 6px; }
  .ntr-meal-badge { font-size: 0.7rem; font-weight: 800; color: #a1a1aa; }
  .ntr-btn-quick-add-meal { background: #27272a; border: 1px solid #3f3f46; color: #38bdf8; border-radius: 6px; padding: 2px 7px; font-size: 0.68rem; font-weight: 800; }

  /* ROW ITEMS (TAP TO EDIT) */
  .ntr-row-item { display: grid; grid-template-columns: 2fr 1fr 1fr 1fr 1fr 1fr 28px; align-items: center; padding: 8px 12px; border-bottom: 1px solid #222226; font-size: 0.76rem; font-weight: 700; color: #f4f4f5; cursor: pointer; }
  .ntr-row-item:hover, .ntr-row-item:active { background: #242429; }
  .ntr-row-item:last-child { border-bottom: none; }

  .col-cals { border-left: 2px solid #f59e0b; padding-left: 8px; font-weight: 800; }
  ...
  .ntr-btn-tool { background: #27272a; color: #e4e4e7; border: 1px solid #3f3f46; border-radius: 8px; padding: 7px 11px; font-size: 0.74rem; font-weight: 700; display: flex; align-items: center; gap: 5px; }
  ...
  .ntr-chip { background: #27272a; border: 1px solid #3f3f46; color: #cbd5e1; font-size: 0.72rem; font-weight: 800; padding: 5px 9px; border-radius: 6px; white-space: nowrap; }
  .ntr-chip:hover { background: #3f3f46; color: #ffffff; }
  ...
  .ntr-modal-box { background: #18181b; border: 1px solid #3f3f46; border-radius: 14px; padding: 18px; width: 92%; max-width: 490px; box-shadow: 0 18px 50px rgba(0,0,0,0.95); max-height: 90vh; overflow-y: auto; -webkit-overflow-scrolling: touch; }
  ...
  .ntr-modal-input { width: 100%; height: 40px; background: #202023; border: 1px solid #2e2e32; color: #fff; border-radius: 8px; padding: 6px 12px; font-weight: 700; box-sizing: border-box; outline: none; margin-bottom: 8px; }
  .ntr-search-results { max-height: 180px; overflow-y: auto; border: 1px solid #27272a; border-radius: 8px; background: #202023; margin-bottom: 10px; -webkit-overflow-scrolling: touch; }
  ...
  .ntr-search-item:hover, .ntr-search-item:active { background: #2a2a30; }
  ...
  .ntr-mini-inp { width: 100%; height: 38px; background: #202023; border: 1px solid #2e2e32; color: #fff; border-radius: 6px; text-align: center; font-weight: 800; box-sizing: border-box; }

  .ntr-del-btn { background: #27272a; border: none; color: #a1a1aa; border-radius: 4px; width: 22px; height: 22px; display: inline-flex; align-items: center; justify-content: center; }
...
function renderMacroTracker() {
  ...
  const isProteinLocked = protPct >= 100;
  const isCalsLocked = Math.abs(remCals) <= (effectiveGoalCals * 0.05);

  // Stacked Segment Bar Calculations
  ...
  const fillBudget = Math.min(100, (totalCals / effectiveGoalCals) * 100);
  ...
  // Visual Water Glasses (10 increments across the target)
  const waterTarget = dayData.goals.water || 3500;
  const currentWater = dayData.water || 0;
  const glassVolume = Math.round(waterTarget / 10);
  let waterGlassesHtml = "";
  for (let g = 1; g <= 10; g++) {
    const isFilled = currentWater >= (g * glassVolume);
    waterGlassesHtml += `
      <div class="ntr-drop-glass ${isFilled ? 'filled' : ''}" data-vol="${g * glassVolume}" title="${g * glassVolume} ml">
        ${isFilled ? '💧' : '🥛'}
      </div>
    `;
  }

  // Meal Grouping: Breakfast, Lunch, Dinner, Post-Workout, Snacks
      ...
      return `
        <div class="ntr-row-item" data-idx="${globalIdx}">
          <div>
            ...
            <div style="font-size:0.68rem; color:#a1a1aa;">${item.serving || "100 g"}</div>
          </div>
          <div></div>
          ...
          <div style="text-align: right;">
            <button class="ntr-del-btn btn-direct-del" data-idx="${globalIdx}" title="Delete item">✕</button>
          </div>
        </div>
      `;
    }).join("");
      ...
      <div class="ntr-meal-card">
        <div class="ntr-meal-header" data-meal="${mealCat}">
          <div class="ntr-meal-title">
            <span class="ntr-arrow ${isCollapsed ? 'closed' : ''}">▼</span>
            <span>${mealCat}</span>
          </div>
          <div style="display:flex; align-items:center; gap:8px;">
            <span class="ntr-meal-badge"><b style="color:#f59e0b;">${mCals.toFixed(0)}</b> kcal • <b style="color:#10b981;">${mProt.toFixed(0)}g</b> P</span>
            <button class="ntr-btn-quick-add-meal btn-quick-add" data-meal="${mealCat}">+ Add</button>
          </div>
        </div>
        <div style="display: ${isCollapsed ? 'none' : 'block'};">
          ${itemsInMealHtml || `<div style="padding:8px 14px; font-size:0.72rem; color:#52525b; text-align:center;">No ${mealCat} foods logged</div>`}
        </div>
      </div>
    `;
  });
  ...
  container.innerHTML = `
    ...
    <div class="ntr-goals-widget ${isProteinLocked || isCalsLocked ? 'goal-achieved-glow' : ''}">
      <div class="ntr-goals-info">
        ...
        ${isProteinLocked ? `<span class="ntr-goal-pill" style="background:#064e3b; color:#a7f3d0;">🛡️ Protein Locked</span>` : ''}
      </div>
      <button class="ntr-btn-edit-goals ntr-btn" id="btn-open-goals-modal">⚙️ Targets</button>
    </div>

    <!-- WORKOUT SYNERGY BADGE (IF TRAINING LOGGED) -->
    ${workoutSplitName ? `
      <div class="ntr-synergy-banner">
        <span>🏋️ <b>Workout Detected:</b> ${workoutSplitName} (${exerciseCaloriesBurned} kcal burned)</span>
        <span style="color:#93c5fd;">⚡ Caloric ceiling dynamically adjusted</span>
      </div>
    ` : ''}

    <!-- MYFITNESSPAL HERO BANNER -->
    ...
    <div class="ntr-stacked-bar-container">
      <div style="display:flex; justify-content:space-between; font-size:0.72rem; font-weight:800;">
        ...
        <span>${totalCals.toFixed(0)} / ${effectiveGoalCals} kcal</span>
      </div>
    ...
    <!-- VISUAL WATER GLASSES PROGRESSION -->
    <div class="ntr-water-dock">
      <div class="ntr-water-top">
        ...
        <div style="display:flex; gap:6px;">
          ...
          <button class="ntr-btn-water ntr-btn" id="w-reset" style="background:#27272a; border-color:#3f3f46; color:#a1a1aa;">↺</button>
        </div>
      </div>
      <div class="ntr-water-glasses-row">${waterGlassesHtml}</div>
    </div>
    ...
    <!-- 2. MEAL-BY-MEAL GROUPED NUTRITION LOG -->
    <div class="ntr-card">
      <div class="ntr-accordion-bar" id="toggle-table">
        <span class="ntr-accordion-title">🍽️ Diary by Meals (Tap Item to Edit)</span>
        <div style="display: flex; align-items: center;">
          <span class="ntr-accordion-cals">${totalCals.toFixed(1)} kcal</span>
      ...
      <div style="display: ${dayData.tableOpen ? 'block' : 'none'};">
        <div class="ntr-action-bar">
          ...
          <button class="ntr-btn-tool ntr-btn" id="btn-open-food-library" style="background:#3b2308; border-color:#d97706; color:#fde68a;">📚 Food Library</button>
          <button class="ntr-btn-tool ntr-btn" id="btn-open-meal-builder" style="background:#1e1b4b; border-color:#4338ca; color:#c7d2fe;">🍱 Combine Meal</button>
          <button class="ntr-btn-tool ntr-btn" id="btn-copy-yesterday" style="background:#172554; border-color:#1d4ed8; color:#bae6fd;">📋 Copy Yesterday</button>
          <button class="ntr-btn-tool ntr-btn" id="btn-open-barcode" style="background:#064e3b; border-color:#059669; color:#a7f3d0;">📷 Scan Barcode</button>
        </div>
    ...
    <!-- 3. MICRONUTRIENTS CARD (COLLAPSED BY DEFAULT) -->
    <div class="ntr-card">
    ...
    <div class="ntr-modal-overlay" id="goals-modal">
      <div class="ntr-modal-box">
        ...
        <div style="display:flex; justify-content:flex-end; gap:8px;">
          <button id="btn-close-goals-modal" class="ntr-btn" style="padding:8px 16px; background:#27272a; border:none; color:#fff; border-radius:8px; font-weight:700;">Cancel</button>
          <button id="btn-save-goals" class="ntr-btn" style="padding:8px 18px; background:#2563eb; border:none; color:#fff; border-radius:8px; font-weight:800;">Save Targets</button>
        </div>
      </div>
    </div>

    <!-- 5. SEARCH & LOG FOOD MODAL (WITH QUICK PORTION CHIPS) -->
    <div class="ntr-modal-overlay" id="ntr-modal">
      <div class="ntr-modal-box">
        ...
        <input type="text" class="ntr-modal-input" id="ntr-search-food" placeholder="🔍 Search library (Sorted by frequency)..." />
        ...
        <div style="display:grid; grid-template-columns: 2fr 1fr; gap:6px;">
          <input type="number" class="ntr-modal-input" id="ntr-custom-serv" placeholder="Grams / Serving" />
        ...
        <div style="display:flex; justify-content:flex-end; gap:8px;">
          <button id="btn-close-ntr-modal" class="ntr-btn" style="padding:8px 16px; background:#27272a; border:none; color:#fff; border-radius:8px; font-weight:700;">Cancel</button>
          <button id="btn-save-ntr-food" class="ntr-btn" style="padding:8px 18px; background:#2563eb; border:none; color:#fff; border-radius:8px; font-weight:800;">+ Log Item</button>
        </div>
      </div>
    </div>

    <!-- 6. INLINE ROW EDIT / ADJUST MODAL -->
    <div class="ntr-modal-overlay" id="edit-row-modal">
      <div class="ntr-modal-box">
        <div class="ntr-modal-title"><span>✏️ Edit Logged Food</span><button class="ntr-del-btn" id="btn-close-edit-x">✕</button></div>
        ...
        <div style="display:flex; justify-content:space-between; gap:8px; margin-top:8px;">
          <button id="btn-delete-active-row" class="ntr-btn" style="padding:8px 14px; background:#450a0a; border:1px solid #7f1d1d; color:#fca5a5; border-radius:8px; font-weight:800;">🗑️ Delete</button>
          <div style="display:flex; gap:8px;">
            <button id="btn-close-edit-modal" class="ntr-btn" style="padding:8px 14px; background:#27272a; border:none; color:#fff; border-radius:8px; font-weight:700;">Cancel</button>
            <button id="btn-save-edited-row" class="ntr-btn" style="padding:8px 18px; background:#2563eb; border:none; color:#fff; border-radius:8px; font-weight:800;">Save Changes</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="ntr-modal-overlay" id="food-library-modal">
      <div class="ntr-modal-box">
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; gap:8px; margin-bottom:8px;">
          ...
          <button id="btn-lib-create-new" class="ntr-btn" style="background:#d97706; color:#fff; border:none; border-radius:8px; padding:0 12px; font-weight:800; font-size:0.75rem; height:40px; white-space:nowrap;">+ New Food</button>
        </div>
        <div class="ntr-search-results" id="lib-foods-list" style="max-height:260px;"></div>
        <div style="display:flex; justify-content:flex-end; margin-top:8px;">
          <button id="btn-close-lib-modal" class="ntr-btn" style="padding:8px 16px; background:#27272a; border:none; color:#fff; border-radius:8px; font-weight:700;">Close</button>
        </div>
      </div>
    </div>
    ...
    <div class="ntr-modal-overlay" id="custom-food-modal">
      <div class="ntr-modal-box">
        ...
        <div style="display:flex; justify-content:flex-end; gap:8px;">
          <button id="btn-close-cf-modal" class="ntr-btn" style="padding:8px 16px; background:#27272a; border:none; color:#fff; border-radius:8px; font-weight:700;">Cancel</button>
          <button id="btn-save-permanent-cf" class="ntr-btn" style="padding:8px 18px; background:#d97706; border:none; color:#fff; border-radius:8px; font-weight:800;">Save Food</button>
        </div>
      </div>
    </div>
    ...
    <div class="ntr-modal-overlay" id="meal-builder-modal">
      <div class="ntr-modal-box">
        ...
        <button id="btn-mb-add-item" class="ntr-btn" style="width:100%; background:#27272a; color:#cbd5e1; border:1px solid #3f3f46; border-radius:8px; padding:8px; font-weight:700; font-size:0.78rem; margin-bottom:10px;">+ Add Ingredient</button>
        ...
        <div style="display:flex; justify-content:flex-end; gap:8px;">
          <button id="btn-close-mb-modal" class="ntr-btn" style="padding:8px 16px; background:#27272a; border:none; color:#fff; border-radius:8px; font-weight:700;">Cancel</button>
          <button id="btn-mb-log-meal" class="ntr-btn" style="padding:8px 18px; background:#4338ca; border:none; color:#fff; border-radius:8px; font-weight:800;">Log Combined Meal</button>
        </div>
      </div>
    </div>

    <!-- 10. BARCODE SCANNER (WITH AUTOMATIC OFFLINE CACHING) -->
    <div class="ntr-modal-overlay" id="barcode-modal">
      <div class="ntr-modal-box">
        <div class="ntr-modal-title"><span>📷 Barcode Scanner (Auto-Cached)</span><button class="ntr-del-btn" id="btn-close-bc-x">✕</button></div>
        ...
        <div style="display:flex; gap:6px; margin-bottom:10px;">
          ...
          <button id="btn-bc-fetch" class="ntr-btn" style="background:#059669; color:#fff; border:none; border-radius:8px; padding:0 14px; font-weight:800; font-size:0.75rem;">Lookup</button>
        </div>
        ...
        <div id="bc-result-preview" style="display:none; background:#202023; border:1px solid #059669; border-radius:8px; padding:10px; margin-bottom:10px;">
          ...
          <button id="btn-bc-use-prod" class="ntr-btn" style="width:100%; background:#059669; color:#fff; border:none; border-radius:6px; padding:8px; font-weight:800; font-size:0.78rem; margin-top:8px;">Add & Save to Library</button>
        </div>

        <div style="display:flex; justify-content:flex-end;">
          <button id="btn-close-bc-modal" class="ntr-btn" style="padding:8px 16px; background:#27272a; border:none; color:#fff; border-radius:8px; font-weight:700;">Close</button>
        </div>
      </div>
    </div>
  `;
  ...
  container.querySelectorAll(".ntr-drop-glass").forEach(gl => {
    gl.onclick = async () => {
      dayData.water = parseInt(gl.dataset.vol, 10);
      await saveNutrition();
      renderMacroTracker();
    };
  });
  ...
  // ==========================================================================
  // MODAL: LOG FOOD WITH FREQUENCY SORTING & QUICK CHIPS
  // ==========================================================================
  ...
  function renderSearchList(query) {
    ...
    // Sort by frequency (usageCount) descending
  ...
  // ==========================================================================
  // MODAL: BARCODE SCANNER (WITH AUTO OFFLINE CACHING)
  // ==========================================================================
  ...
  async function fetchOpenFoodFacts(barcode) {
    // Check if already in custom foods library first (Instant offline cache)
    const cached = customFoods.find(f => f.barcode === barcode);
    if (cached) {
      scannedProductData = { ...cached };
      bcProdName.textContent = `⚡ (Offline Cached) ${cached.name}`;
      ...
      bcStatus.textContent = "Product loaded instantly from offline cache!";
      stopCamera();
      return;
    }

    bcStatus.textContent = `Connecting to Open Food Facts (${barcode})...`;
    try {
      ...
      if (data.status === 1 && data.product) {
        ...
        // Auto-save to custom-foods.json for offline access next time
        ...
        bcPreview.style.display = "block";
        bcStatus.textContent = "Product identified and cached for offline use!";
        stopCamera();
      } else {
        bcStatus.textContent = "❌ Product not found. Try manual food entry.";
      }
    } catch (err) {
      bcStatus.textContent = "❌ Network error. Check connection or enter manually.";
    }
  }
  ...
  async function startCamera() {
    ...
    try {
      ...
      if ('BarcodeDetector' in window) {
        ...
        bcStatus.textContent = "Camera active. (Enter barcode below if browser lacks BarcodeDetector)";
      }
    } catch (e) {
