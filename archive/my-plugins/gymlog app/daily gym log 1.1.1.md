...
style.textContent = `
  ...
  .wk-recap-btn-row { display: flex; gap: 10px; margin-top: 16px; }
  .wk-btn-edit { flex: 1.2; background: #2563eb; color: #ffffff; border: none; border-radius: 10px; padding: 12px; font-weight: 800; font-size: 0.88rem; cursor: pointer; transition: all 0.15s ease; text-align: center; }
  .wk-btn-edit:hover { background: #1d4ed8; }
  .wk-btn-reset { flex: 0.8; background: #181124; border: 1px solid #4f2334; color: #fca5a5; border-radius: 10px; padding: 12px; font-weight: 800; font-size: 0.88rem; cursor: pointer; transition: all 0.15s ease; text-align: center; }
  .wk-btn-reset:hover { background: rgba(239,68,68,0.22); border-color: #ef4444; color: #f87171; }
...
function renderFinishedScreen(data) {
  ...
  trackerApp.innerHTML = `
    <div class="wk-recap-screen">
      ...
      <div>${cardsHtml}</div>
      <div class="wk-recap-btn-row">
        <button class="wk-btn-edit" id="btn-edit-session">✏️ Edit Current Session</button>
        <button class="wk-btn-reset" id="btn-clear-reset">🗑️ Clear & Start Fresh</button>
      </div>
    </div>
  `;

  trackerApp.querySelector("#btn-edit-session").onclick = () => {
    sessionExercises = JSON.parse(JSON.stringify(data.exercises || []));
    activeSplitCategory = data.split || "Push";
    initActiveApp();
  };

  trackerApp.querySelector("#btn-clear-reset").onclick = async () => {
    ...
    if (currentFile) {
      await app.fileManager.processFrontMatter(currentFile, fm => {
        delete fm["workout_split"];
        delete fm["workout_volume"];
        delete fm["workout_sets"];
        delete fm["workout_calories"];
      });
    }
    sessionExercises = [];
...
function initActiveApp() {
  ...
  function showPlateCalculator(weight) {
    ...
    if (w < 20) {
      plateText.textContent = `Added Load: ${w} kg`;
