...
if (!nutritionDB[todayKey].recovery) {
  nutritionDB[todayKey].recovery = {
    ...
    quality: 4,
    soreness: 2,
    supps: { mg: true, gly: false, mel: false }
  };
}
if (!nutritionDB[todayKey].recovery.supps) {
  nutritionDB[todayKey].recovery.supps = { mg: true, gly: false, mel: false };
}
...
function computeMetrics() {
  ...
  const supps = rec.supps || { mg: false, gly: false, mel: false };
...
style.textContent = `
  ...
  .rec-sq-cell:active { transform: scale(0.95); }
  .rec-sq-cell.active {
...
function renderUI() {
  const { score, hours, quality, soreness, supps, estDeepSleepMins, ghPeakUnits, history7d, badgeText, badgeClass, targetRirAdvice } = computeMetrics();
  ...
  widgetContainer.innerHTML = `
    <div class="rec-night-card">
      ...
      <!-- SUPPLEMENT LOGGING CHIPS -->
      <div style="display:flex; justify-content:space-between; align-items:center; background:#150e21; border:1px solid #281d3d; border-radius:5px; padding:5px 8px; margin-bottom:8px;">
        <span style="font-size:0.67rem; color:#a78bfa; font-weight:700;">Night Stack:</span>
        <div style="display:flex; gap:5px;">
          <span class="rec-supp-chip ${supps.mg ? 'active' : ''}" id="btn-supp-mg">💊 Magnesium</span>
          <span class="rec-supp-chip ${supps.gly ? 'active' : ''}" id="btn-supp-gly">🧪 Glycine</span>
          <span class="rec-supp-chip ${supps.mel ? 'active' : ''}" id="btn-supp-mel">🌙 Melatonin</span>
        </div>
      </div>
  ...
  // Supplement Toggle Handlers
  widgetContainer.querySelector("#btn-supp-mg").onclick = async () => {
    nutritionDB[todayKey].recovery.supps.mg = !nutritionDB[todayKey].recovery.supps.mg;
    await saveRecoveryData();
    renderUI();
  };

  widgetContainer.querySelector("#btn-supp-gly").onclick = async () => {
    nutritionDB[todayKey].recovery.supps.gly = !nutritionDB[todayKey].recovery.supps.gly;
    await saveRecoveryData();
    renderUI();
  };

  widgetContainer.querySelector("#btn-supp-mel").onclick = async () => {
