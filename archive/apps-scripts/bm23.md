...
styleEl.textContent = `
    ...
    @keyframes bm3-zone-pulse {
        0%   { transform: rotate(var(--tilt, 0deg)) scale(1.018); }
    ...
    .bm3-zone-hit.selected {
        animation: bm3-zone-pulse 0.24s ease-out;
    }
            ...
            zoneDef.bands.forEach((band, idx) => {
                ...
                defs += `
                    <pattern id="${pid}" width="6" height="6" patternTransform="rotate(${band.angle})" patternUnits="userSpaceOnUse">
                        ...
                        <line x1="0" y1="0" x2="0" y2="6" stroke="#000000" stroke-width="0.95" stroke-opacity="0.8" />
                        <line x1="3" y1="0" x2="3" y2="6" stroke="#ffffff" stroke-width="0.55" stroke-opacity="0.45" />
                    </pattern>
                `;
            });
        } else {
...
// 9. ANNOTATION — FIX: opaque backdrop behind text, moderate distance
...
function showAnnotation(svgEl, view, point, label) {
    ...
    // Compute approximate text bbox for backdrop
    const textLength = label.length * 14; // rough estimate
    const backdropPad = 8;
...
// NEW: Split each band into per-side rects so each side gets its own center
function computeBandsPerSide(bandDefs, pathElements, axis) {
    const result = {};
    pathElements.forEach(p => {
        const bbox = p.getBBox();
        const bboxId = `${bbox.x}|${bbox.y}|${bbox.width}|${bbox.height}`;
        
        if (!result[bboxId]) {
            let cum = 0;
            const overflow = axis === "vertical" ? bbox.width * 0.9 : bbox.height * 0.9;
            result[bboxId] = bandDefs.map((b, idx) => {
                const start = cum, end = cum + b.frac;
                cum = end;
                const tilt = b.angle * 0.35;
                if (axis === "vertical") {
...
function renderBody(view) {
    ...
    // --- Phase B: build real clipped sub-zones with per-side band calculation ---
    for (const [key, item] of Object.entries(muscleRegistry)) {
        if (item.view !== view) continue;
        const zoneDef = ZONE_DEFS[key];
        if (!zoneDef) continue;
        ...
        // Each path gets its own per-side band calculation
        const bandsByPath = computeBandsPerSide(zoneDef.bands, baseEls, zoneDef.axis);
        
        let zonesHtml = `<g clip-path="url(#${clipId})">`;
        
        baseEls.forEach((p, pathIdx) => {
            const bbox = p.getBBox();
            const bboxId = `${bbox.x}|${bbox.y}|${bbox.width}|${bbox.height}`;
            const bands = bandsByPath[bboxId] || [];
