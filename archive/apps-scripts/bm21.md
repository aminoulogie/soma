...
styleEl.textContent = `
    ...
    .bm3-zone-texture { pointer-events: none; mix-blend-mode: overlay; opacity: 0.5; }
    ...
    /* Selection cue for a zone is drawn as an outline on the muscle's real
       ...
       what was causing the ugly internal divider line between bands. */
    .bm3-zone-outline {
        pointer-events: none;
        fill: none;
        stroke: #38bdf8;
    ...
    .bm3-muscle-group.selected,
    .bm3-zoneband.selected {
        transform-box: fill-box;
        transform-origin: center;
        animation: bm3-pulse 0.32s ease-out;
    }
...
function showAnnotation(svgEl, view, point, label) {
    ...
    const html = `
        <g class="bm3-annot-group">
            <line x1="${cx}" y1="${cy}" x2="${targetX}" y2="${cy}" stroke="#38bdf8" stroke-width="3" stroke-linecap="round" />
            <circle class="bm3-annot-dot" cx="${cx}" cy="${cy}" r="7" fill="#38bdf8" stroke="#0b1220" stroke-width="2.5" />
            <text x="${textX}" y="${cy}" fill="#f8fafc" font-size="23" font-weight="800"
...
// Converts a real click event into exact SVG user-space coordinates, so the
// annotation dot lands precisely where the finger/cursor actually was —
// instead of the bounding-box center, which can sit outside an odd shape.
function getClickSVGPoint(evt, svgEl, fallbackEl) {
    try {
...
function renderBody(view) {
    ...
    for (const [key, item] of Object.entries(muscleRegistry)) {
        ...
        const pathClass = `bm3-muscle-path${isSelected ? (showAsSelectedInactive ? " selected selected-inactive" : " selected") : ""}`;
    ...
    for (const [key, item] of Object.entries(muscleRegistry)) {
        ...
        bands.forEach(band => {
            ...
            zonesHtml += `<g class="bm3-zoneband${isSel ? " selected" : ""}">`;
        ...
        zonesHtml += `</g>`;
        // Whole-muscle contour outline in blue when one of its zones is
        // selected — drawn on the real (unclipped) outline, so there's no
        // hard rectangle edge shown between adjacent bands.
        if (selectedZone && selectedZone.key === key) {
            zonesHtml += item.paths.map(p => `<path class="bm3-zone-outline" d="${p}" />`).join("");
        }
    ...
    svgEl.querySelectorAll(".bm3-muscle-path").forEach((p) => {
        ...
        p.addEventListener("click", (e) => {
            selectedKey = key;
            selectedZone = null;
            selectedPoint = getClickSVGPoint(e, svgEl, p);
    ...
    svgEl.querySelectorAll(".bm3-zone-hit").forEach((r) => {
        ...
        r.addEventListener("click", (e) => {
            ...
            selectedKey = null;
            selectedPoint = getClickSVGPoint(e, svgEl, r);
            renderBody(currentView);
