...
const muscleRegistry = {
  delts_front: {
    ...
    defaultHours: 36,
    tier: "Fast Recovery",
    desc: "Primary shoulder flexion and pressing contribution. Highly loaded during overhead and incline presses."
  },
  delts_side: {
    ...
    defaultHours: 24,
    tier: "Ultra-Fast",
    desc: "Responsible primarily for shoulder abduction and capped shoulder width. Tolerates high frequency."
  },
  chest_upper: {
  ...
  abs_core: {
    ...
    defaultHours: 24,
    tier: "Ultra-Fast",
    desc: "Primary trunk flexion, serratus protraction, and intra-abdominal pressure stabilization."
  },
  quads: {
    ...
    defaultHours: 72,
    tier: "High Systemic",
    desc: "Major knee-extension group (rectus femoris, vastus medialis/lateralis) causing high systemic fatigue."
  },
  calves_ant: {
    name: "Tibialis Anterior & Lower Leg",
    region: "Lower Leg",
    defaultHours: 24,
    tier: "Ultra-Fast",
    desc: "Anterior lower-leg musculature involved in ankle dorsiflexion and knee deceleration."
  },
  traps_upper: {
    ...
    defaultHours: 48,
    tier: "Standard",
    desc: "Upper-back musculature involved in scapular retraction, elevation, and spinal stabilization."
  },
  delts_rear: {
    ...
    defaultHours: 24,
    tier: "Ultra-Fast",
    desc: "Rear shoulder musculature contributing to horizontal pulling, external rotation, and rear delt flyes."
  },
  lats: {
    ...
    defaultHours: 48,
    tier: "Standard",
    desc: "Large sweeping back muscle responsible for vertical pulling, shoulder extension, and V-taper sweep."
  },
  triceps: {
  ...
  lower_back: {
    ...
    defaultHours: 72,
    tier: "High Axial",
    desc: "Spinal extension and trunk column stability. High axial fatigue requiring extended recovery."
  },
  glutes: {
    ...
    defaultHours: 48,
    tier: "Standard",
    desc: "Major hip-extension musculature loaded heavily during deep squats, deadlifts, and hip thrusts."
  },
  hamstrings: {
    ...
    defaultHours: 72,
    tier: "High Systemic",
    desc: "Posterior thigh group (biceps femoris, semitendinosus) with high stretch-induced muscle damage."
  },
  calves_post: {
...
.bodymap-container {
  ...
  box-shadow: 0 28px 75px rgba(0, 0, 0, 0.7), inset 0 1px 0 rgba(255, 255, 255, 0.05);
}

.bodymap-header { text-align: center; margin-bottom: 22px; }
.bodymap-title { margin: 0; color: #f8fafc; font-size: 1.4rem; font-weight: 850; letter-spacing: -0.025em; }
...
.bodymap-side-row { display: flex; align-items: flex-start; gap: 10px; margin-bottom: 14px; }
...
.dot-red { background: #ff2e3b; box-shadow: 0 0 12px rgba(255,46,59,0.7); }
.dot-orange { background: #ff7415; box-shadow: 0 0 12px rgba(255,116,21,0.65); }
.dot-yellow { background: #ffca00; box-shadow: 0 0 12px rgba(255,202,0,0.6); }
.dot-green { background: #44cc44; box-shadow: 0 0 12px rgba(68,204,68,0.5); }
.dot-bluegray { background: #223247; }
...
.how-icon {
  ...
  border-radius: 50%;
  color: #c8d5e8;
  background: rgba(31, 47, 72, 0.65);
  border: 1px solid rgba(92, 114, 153, 0.3);
  font-size: 0.78rem;
}
...
.bodymap-svg { width: 100%; height: 440px; overflow: visible; display: block; filter: drop-shadow(0 12px 28px rgba(0,0,0,0.65)); }

/* 3D Anatomical Base Anatomy Shading */
.anat-head { fill: url(#anatHeadGrad); stroke: #080d16; stroke-width: 1.2; }
.anat-frame { fill: url(#anatBaseGrad); stroke: #090f1a; stroke-width: 1.1; }
.anat-shadow { fill: #0b121e; stroke: #060a12; stroke-width: 1; }
.anat-fiber { fill: none; stroke: rgba(255, 255, 255, 0.08); stroke-width: 0.9; pointer-events: none; }

/* Interactive Muscle Layer */
.muscle {
  fill: url(#muscleDefaultGrad);
  stroke: #070c16;
  stroke-width: 1.25;
  cursor: pointer;
...
.muscle:hover {
  fill: url(#muscleHoverGrad) !important;
  filter: drop-shadow(0 0 8px #38bdf8) !important;
}

/* 3D Radiant Heatmap Glowing States */
.muscle.sore-max {
  fill: url(#soreMaxGrad) !important;
  filter: drop-shadow(0 0 6px rgba(255, 46, 59, 0.95)) drop-shadow(0 0 16px rgba(255, 46, 59, 0.5));
}
.muscle.sore-mid {
  fill: url(#soreMidGrad) !important;
  filter: drop-shadow(0 0 6px rgba(255, 116, 21, 0.9)) drop-shadow(0 0 15px rgba(255, 116, 21, 0.45));
}
.muscle.sore-low {
  fill: url(#soreLowGrad) !important;
  filter: drop-shadow(0 0 5px rgba(255, 202, 0, 0.85)) drop-shadow(0 0 13px rgba(255, 202, 0, 0.4));
}
.muscle.fresh {
  fill: url(#soreFreshGrad) !important;
  filter: drop-shadow(0 0 6px rgba(68, 204, 68, 0.85));
}
...
const svgDefs = `
<defs>
  <radialGradient id="anatHeadGrad" cx="44%" cy="30%" r="68%">
    <stop offset="0%" stop-color="#2c3e5c"/>
    <stop offset="55%" stop-color="#141e30"/>
    <stop offset="100%" stop-color="#080e18"/>
  </radialGradient>

  <linearGradient id="anatBaseGrad" x1="0" y1="0" x2="1" y2="1">
    <stop offset="0%" stop-color="#1c283c"/>
    <stop offset="50%" stop-color="#121b2a"/>
    <stop offset="100%" stop-color="#090f1a"/>
  </linearGradient>

  <linearGradient id="muscleDefaultGrad" x1="0" y1="0" x2="0" y2="1">
    <stop offset="0%" stop-color="#1e2d46"/>
    <stop offset="100%" stop-color="#121d2f"/>
  </linearGradient>

  <linearGradient id="muscleHoverGrad" x1="0" y1="0" x2="0" y2="1">
    <stop offset="0%" stop-color="#324e78"/>
    <stop offset="100%" stop-color="#1c2f4c"/>
  </linearGradient>

  <linearGradient id="soreMaxGrad" x1="0" y1="0" x2="0" y2="1">
    <stop offset="0%" stop-color="#ff4a55"/>
    <stop offset="50%" stop-color="#ef2333"/>
    <stop offset="100%" stop-color="#a80b18"/>
  </linearGradient>

  <linearGradient id="soreMidGrad" x1="0" y1="0" x2="0" y2="1">
    <stop offset="0%" stop-color="#ff8d36"/>
    <stop offset="50%" stop-color="#ff6b08"/>
    <stop offset="100%" stop-color="#b84500"/>
  </linearGradient>

  <linearGradient id="soreLowGrad" x1="0" y1="0" x2="0" y2="1">
    <stop offset="0%" stop-color="#ffdf43"/>
    <stop offset="50%" stop-color="#ffbe00"/>
    <stop offset="100%" stop-color="#ba8200"/>
  </linearGradient>

  <linearGradient id="soreFreshGrad" x1="0" y1="0" x2="0" y2="1">
    <stop offset="0%" stop-color="#55e655"/>
    <stop offset="50%" stop-color="#34b834"/>
    <stop offset="100%" stop-color="#197519"/>
  </linearGradient>
</defs>
`;
...
<svg class="bodymap-svg" viewBox="0 0 200 480" xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)">
  ...
  <!-- Silhouette Skeleton Layer -->
  <ellipse class="anat-head" cx="100" cy="28" rx="17" ry="22"/>
  <path class="anat-frame" d="M88 46 C90 56 88 62 82 68 L118 68 C112 62 110 56 112 46 Z"/>
  <path class="anat-shadow" d="M86 52 C88 62 94 66 100 68 C106 66 112 62 114 52 Z"/>
  ...
  <path class="anat-fiber" d="M85 144 L115 144 M83 160 L117 160"/>
  ...
  <circle class="anat-shadow" cx="81" cy="284" r="5"/>
  <circle class="anat-shadow" cx="119" cy="284" r="5"/>
  ...
  <!-- Feet -->
  <path class="anat-frame" d="M74 376 C70 378 67 386 70 393 C78 397 92 397 97 391 L94 379 Z"/>
  <path class="anat-frame" d="M126 376 C130 378 133 386 130 393 C122 397 108 397 103 391 L106 379 Z"/>
</svg>
`;
...
<svg class="bodymap-svg" viewBox="0 0 200 480" xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)">
  ...
  <circle class="anat-shadow" cx="82" cy="301" r="4.5"/>
  <circle class="anat-shadow" cx="118" cy="301" r="4.5"/>
...
mapContainer.innerHTML = `
  ...
  <div class="bodymap-detail-card">
    <div class="detail-top">
      ...
      <div class="detail-badge" id="det-badge" style="background:#ff2e3b; color:#ffffff;">
...
function updateInspector(partKey) {
  ...
  if (isMax || isMid || isLow) {
    ...
    if (isMax) detBadge.style.background = "#ff2e3b";
    else if (isMid) detBadge.style.background = "#ff7415";
    else detBadge.style.background = "#ffca00";
