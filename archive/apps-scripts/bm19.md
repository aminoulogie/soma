...
// Unique per-execution prefix. Obsidian can briefly keep two instances of this
// block alive in the document at once during an edit<->reading transition. If
// both instances used the same gradient/pattern ids ("grad-front-chest"), a
// url(#id) reference can resolve to the wrong (or a removed) element and the
// muscle fill silently fails to paint - this was the real cause of FRONT only
// working in edit mode. Every id below is namespaced with this so collisions
// are impossible regardless of how many copies of the block exist at once.
...
function renderBody(view) {
    ...
    // Reading-mode fix: Obsidian sometimes attaches this container to the live
    // document a tick after the script runs. If the SVG is painted while still
    // detached, the <defs> gradients/patterns referenced via url(#id) can fail
    // to bind on first paint. Forcing a reflow (+ a rAF re-poke) fixes it.
...
// Deferred first paint: in Reading mode the container can still be off-DOM
// the instant this script finishes running, which is what breaks the very
// first FRONT render. Waiting a frame guarantees it's attached first.
requestAnimationFrame(() => {
    renderBody("front");
    renderDetails("chest");
});
